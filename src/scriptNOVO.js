$(document).ready(function () {
  ...
  console.log(imageWrapperChildren[1]);
  ...
  alert(
    imageWrapperChildren.map(function (index, x) {
      imageWrapperChildren[x].slice(0, column[x]).forEach((item) => {
        imageWrapper[x].prepend($(item));
      });
    })
  ); //adds a new img element after the array

  imageWrapperChildren[x].slice(-column[x]).forEach((item) => {
    imageWrapper[x].append($(item));
  });
  //adds a new img element before the array

  //imageWrapper[x].append(imageWrapperChildren);

  imageWrapperChildren.map(function (index, imageWrapper) {
