...
$(document).ready(function () {
  ...
  var imageWrapperChildren = [$(".image-wrapper").children().toArray(), $(".image-wrapper-bottom").children().toArray()]; //making an array of image wrapper children
  var widthToScroll = imageWrapperChildren[0][0].offsetWidth;
  var column = [Math.floor(imageWrapper[0].offsetWidth / widthToScroll), Math.floor(imageWrapper[1].offsetWidth / widthToScroll)];
  var _loop = function _loop(x) {
    imageWrapperChildren[x].slice(0, column[x]);
    imageWrapperChildren[x].forEach(function (item) {
      imageWrapper[x].insertAdjacentHTML("beforeend", $(item).prop("outerHTML"));
    }); //adds a new img element after the array

    imageWrapperChildren[x].slice(-column[x]).reverse().forEach(function (item) {
      imageWrapper[x].insertAdjacentHTML("afterbegin", $(item).prop("outerHTML"));
    ...
    function loopFunc(z) {
      if (imageWrapper[x].scrollLeft < z) {
        ...
        imageWrapper[x].scrollLeft = imageWrapper[x].scrollWidth - 2 * imageWrapper[x].offsetWidth;
        imageWrapper[x].classList.remove("no-smooth");
      } else if (imageWrapper[x].scrollLeft === imageWrapper[x].scrollWidth - imageWrapper[x].offsetWidth) {
        imageWrapper[x].classList.add("no-smooth");
